async function redeemprocess() {
        ...
        return new Promise(async (res) => {
            ...
            try {
                ...
                await Page.evaluate(() => Array.from(document.querySelectorAll('button[type=button]'), async (element) => {
                    if (element.textContent == "Next" || element.textContent == "İleri" || element.textContent == "Sonraki")
                        await element.click()
                }));
                await wait(1500)
                ...
                await Page.waitForSelector('button[type=button]');
                await Page.evaluate(() => Array.from(document.querySelectorAll('button[type=button]'), async (element) => {
                    if (element.textContent == "Next" || element.textContent == "İleri")
                        await element.click()
                }));

                await Page.waitForSelector('button[type=submit]');
                await Page.evaluate(() => Array.from(document.querySelectorAll('button[type=submit]'), async (element) => {
                    if (element.textContent.includes("Get Nitro Monthly") || element.textContent.includes("Aylık Nitro al"))
                        await element.click()
                }));
                ...
                const alertspeaker = require('speaker');
                const soundBuffer = fs.readFileSync('datas/redeemer-data/alert.pcm');
                const speaker = new alertspeaker();

                speaker.write(soundBuffer);

                console.log("KOD GİRİŞİ YAPINIZ!")
                console.log("KOD GİRİŞİ YAPINIZ!")
                console.log("KOD GİRİŞİ YAPINIZ!")
                console.log("KOD GİRİŞİ YAPINIZ!")
                console.log("KOD GİRİŞİ YAPINIZ!")
                ...
                try {
                    await Page.waitForFunction(
                        'document.querySelector("body").innerText.includes("Başarısız")',
                        {
                            timeout: 20 * 1000
                        }
                    );
                ...
                await Page.waitForFunction(
                    'document.querySelector("body").innerText.includes("Enjoy friend!") || document.querySelector("body").innerText.includes("Keyfini çıkar dostum!")',
                    {
                        timeout: 3 * 60 * 1000
                    }
                );
                ...
                fs.appendFile('datas/joiner-data/process-tokens.txt', `${token}\n`, 'utf8', (error) => {
                    if (error) {
                        ...
                        console.log(`Token "${token}" dosyaya başarıyla eklendi.`);
                    }
                });
    ...
    (async () => {
        ...
        console.log("Total " + cards.length + " Started!")
        /*
        ...
        for (var i = 0; i < cards.length; i++) {
            ...
            if (db.has(`already_${tkn}`)) {
                console.log(`${(i + 1)}/` + cards.length + ' already next..');
                continue;
            }
            await start(i, tkn, nitro, proxy, card, { now: (i + 1), max: cards.length });

            await wait(3000);
        }
        console.log("All Tokens Finished!" + cards.length);
    })();
}
